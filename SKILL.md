...
### 5b. Product UI/UX Baseline (HIGH)

- `task-first-screen` - Put the primary user task in the first viewport; avoid landing-page filler for tools, dashboards, admin panels, news hubs, or community portals
- `route-model` - Use clear listing/detail routes for repeatable content; every detail page must be deep-linkable and have a not-found state
- `hidden-admin` - Keep admin routes out of public navigation; protect them with server-side auth, HttpOnly sessions, and server-only env vars
- `content-hierarchy` - Homepage shows curated highlights; full listing pages show all records; detail pages handle article metadata, body hierarchy, related content, and back navigation
- `stable-layout` - Reserve dimensions for navbars, logos, cards, image frames, stats, and async content to prevent layout shift
- `admin-workflow` - Admin UI must support create, edit, save feedback, validation errors, delete/hide behavior, empty states, expired sessions, and direct view/preview links
- `database-bounds` - CMS/database content must be normalized and bounded so long text, too many highlights, or missing image URLs cannot break layout or exhaust storage
- `real-media` - Use real product/community imagery when the UI represents a specific brand, server, product, venue, or person; avoid purely decorative abstract visuals
- `surface-discipline` - Use cards only for repeated items, modals, and framed tools; keep page sections as full-width bands or unframed layouts
- `manual-route-check` - Before delivery, verify home, listing, detail, admin, and 404 routes plus one mobile viewport
